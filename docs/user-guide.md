...
命令会复制快照、做 `integrity_check`、从原数据目录把 `logos/` 一并带上，并核对条目引用的图标是否在位；之后把 `KALENDS_DATA`（或 compose 的数据卷）指向新目录即可。退出码 `0` 为完整恢复；`1` 表示数据库完好但有引用文件缺失（会逐个列出）。目标目录必须为空——恢复永不覆盖在用数据。
...
- **v0.1.x → v0.2.0**：媒体库已迁往 [Ludi](https://github.com/Lynthar/Ludi)。**还有媒体数据时新版本会拒绝启动**（错误里点名原因）——先用 Ludi 仓库的 `scripts/import-from-kalends.py` 把媒体数据整份搬走并验收，或在旧版本界面清空媒体库，再升级。升级后数据目录里的 `covers/` 不再被使用，确认 Ludi 侧无恙后可手动删除。
...
1. **设置页「通知发送记录」**：每次投递成败都记一条，失败带原因（悬停「失败」看全文）。这里空着说明决策层就没发——往下查。失败后按 30 分钟 / 1 / 2 / 4 小时退避重试，第五次仍失败就放弃这一条（修好渠道后，下一个提醒档位与次日摘要照常发）。
...
- 数据目录（db + logos + backups + export）纳入主机的整机备份即可；应用自身每日 03:30 做快照轮转与 JSONL 明文导出。
