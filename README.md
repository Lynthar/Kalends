...
- 建库时可以挑模板（域名 / 保险 / 证件），字段与到期模型直接就位；模板落地后就是普通字段，照样能改名、改选项、删掉
- 原币记账（CNY/USD/EUR… 各算各的，不强制折算）、涨价历史、"服务 → 套餐档位"父子结构可折叠、每个条目能配图标
- 状态不只是个标签，而是带含义的：每个状态值自己声明计不计支出、发不发提醒、上不上时间线（表头菜单里改）。所以 `Deferred` 可以当比价目录（三样都不算），`Ending` 可以留在日历里但不再吵你
- 库的顺序拖标签就能改；字段的顺序和「上不上表格」在库设置里拖，跟着账本走而不是跟着这台设备走
